...
--@@ Output = lazada_analyst_dev.tmp_loutruong_aff_console_di
--@@ Output = lazada_analyst_dev.loutruong_aff_console_di
----------------------------------------------------------------------------------------------------------------------------
-- Build up raw at the lowest level of partner is pid and transaction is sales_order_item_id
----------------------------------------------------------------------------------------------------------------------------
-- DROP TABLE IF EXISTS lazada_analyst_dev.tmp_loutruong_aff_console_di
-- ;
-- CREATE TABLE IF NOT EXISTS lazada_analyst_dev.tmp_loutruong_aff_console_di
-- (
...
-- PARTITIONED BY 
-- (
--     ds                            STRING COMMENT 'order_create_date'
-- )
-- LIFECYCLE 3600
-- ;
INSERT OVERWRITE TABLE lazada_analyst_dev.tmp_loutruong_aff_console_di PARTITION (ds)
SELECT t1.sales_order_item_id AS sales_order_item_id,
    ...
    t1.order_create_hh AS hh,
    t1.order_create_date AS ds
FROM (
        SELECT TO_CHAR(
        ...
        WHERE 1 = 1 --
            ...
            AND venture = 'VN' --
            -- AND     TO_CHAR(TO_DATE(conversion_time,'yyyy-mm-ddTHH:mi'),'yyyymmdd') >= '20220101' --<< Start
            AND TO_CHAR(
                TO_DATE(conversion_time, 'yyyy-mm-ddTHH:mi'),
                'yyyymmdd'
            ) >= TO_CHAR(
...
----------------------------------------------------------------------------------------------------------------------------
-- DROP TABLE IF EXISTS lazada_analyst_dev.loutruong_aff_console_di
-- ;
-- CREATE TABLE IF NOT EXISTS lazada_analyst_dev.loutruong_aff_console_di
-- (
--     sales_order_item_id           STRING
--     ,order_id                     STRING
--     ,order_number                 STRING
...
-- LIFECYCLE 3600
-- ;
INSERT OVERWRITE TABLE lazada_analyst_dev.loutruong_aff_console_di PARTITION (ds)
SELECT t1.sales_order_item_id AS sales_order_item_id,
...
FROM (
        SELECT sales_order_item_id,
            ...
            hh,
            ds
        FROM lazada_analyst_dev.tmp_loutruong_aff_console_di
    ) AS t1
    LEFT JOIN (
