            ...
            SELECT sales_order_item_id,
                ...
                MAX(
                    CASE
                        ...
                        AND ABS(COALESCE(KEYVALUE(exp_comm_amt_detail, 'LPI'), 0)) > 0 THEN 1
                        ELSE 0
                    END
...
,
                ...
                SUM(
                    CASE
                        WHEN ds BETWEEN 20240401 AND 20240930 THEN ABS(COALESCE(KEYVALUE(exp_comm_amt_detail, 'LPI'), 0))
                        ELSE 0
                    END
...
,
                SUM(
                    CASE
                        WHEN ds >= 20241001 THEN ABS(COALESCE(KEYVALUE(exp_comm_amt_detail, 'LPI'), 0))
                        ELSE 0
                    END
...
SELECT t1.ds AS ds,
    TO_CHAR(
        TO_DATE(t1.fulfillment_create_date, 'yyyy-mm-dd hh:mi:ss'),
        'hh'
    ) AS hh,
